...
double RTau3pi::genFlat (std::random_device & rd, double min=-1., double max=1.)
{
  if (min > max)
  {
    double tmp=max;
...
double RTau3pi::px (double e1, double e2, double p3, double m, int i=1)
{
  double sign =1.;
  if (i%2==0)
    sign=-1.;
...
RParticleList RTau3pi::genDecay (RVector & Ptau, RVector & Xtau)
{
  RParticleList plist;
